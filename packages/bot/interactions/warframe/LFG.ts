...
import { cmds, platformMap as platformChoices } from '#shared/resources/index';
import LFGEmbed from '#shared/embeds/LFGEmbed';
...
export default class LFG extends Interaction {
  ...
  static async commandHandler(interaction, ctx) {
    ...
    if (!rawChn) return interaction.reply(withEphemeral(ctx.ephemerate, { content: ctx.i18n`Couldn't find channel.` }));
    const chn = interaction.guild.channels.resolve(rawChn.id);
    if (!chn) return interaction.reply(withEphemeral(ctx.ephemerate, { content: ctx.i18n`Couldn't find channel.` }));

    const buttons = [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setStyle(ButtonStyle.Primary).setCustomId('lfg_add').setEmoji('🔰').setLabel('Join'),
        new ButtonBuilder().setStyle(ButtonStyle.Danger).setCustomId('lfg_end').setEmoji('❌').setLabel('End')
      ),
    ];
    ...
    if (!message) {
      return interaction.reply(withEphemeral(true, { content: 'Unknown error. Could not create LFG entry.' }));
    }
