...
const pkg = JSON.parse(readFileSync(resolve(findMonorepoRoot(), 'package.json'), 'utf8')) as { version: string };
...
export default class About extends Interaction {
  ...
  static commandHandler = async (interaction: ChatInputCommandInteraction) => interaction.reply(
    withEphemeral(true, {
      embeds: [
        {
