...
export default class VoidTraderEmbed extends BaseEmbed {
  constructor(voidTrader, { platform, onDemand, i18n, locale }) {
    ...
    this.setColor(voidTrader?.active ? 0x0ec9ff : 0xff6961);

    if (voidTrader?.active && voidTrader?.inventory?.length > 0) {
      this.setFields(
        voidTrader?.inventory.map((i) => {
    ...
    this.setTitle(i18n`[${platform.toUpperCase()}] Worldstate - Void Trader`);
    this.setThumbnail(baroThumb);
  }
}
