...
class AddBlacklist extends Command {
  ...
  constructor(logger, { mdConfig = md, regexPrefix = '\/', prefix = '/' } = {}) {
    ...
    this.commandId = 'genesis.blacklist.add';
    // eslint-disable-next-line no-useless-escape
    this.commandRegex = new RegExp(`^${regexPrefix}disable (.+)`, 'i');
    this.commandHelp = `${prefix}disable command | Disables a command [Admin only]`;
    this.logger = logger;
    ...
    settings.getBlacklistedCommands(message.channel.id)
        .then((blacklist) => {
          if ((blacklist.indexOf(query) === -1 && blacklistables.indexOf(query) !== -1) || query === 'all') {
            settings.setBlacklistCommand(message.channel.id, query, true)
              .then((r) => {
                this.logger.debug(r);
                if (query !== 'all') {
                  stringManager.getString('command_added_blacklist', message, { command: query })
                    .then((str) => {
                      message.reply(str)
                        .then((msg) => {
                          if (msg.deletable) {
                  ...
                  stringManager.getString('all_command_added_blacklist', message, { command: query })
                    .then((str) => {
                      message.reply(str)
                        .then((msg) => {
                          if (msg.deletable) {
