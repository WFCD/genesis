...
import BuildEmbed from '#shared/embeds/BuildEmbed';
import FrameEmbed from '#shared/embeds/FrameEmbed';
import WeaponEmbed from '#shared/embeds/WeaponEmbed';
...
export default class Builds extends Interaction {
  ...
  static async commandHandler(interaction, ctx) {
    ...
    let build = await ctx.settings.builds.getBuild(query || id);
    switch (action) {
      case 'list':
        const builds = await ctx.settings.builds.getBuilds(false, interaction.user);
        if (builds.length > 0) {
        ...
        return interaction.reply(
          withEphemeral(ctx.ephemerate, { embeds: [{ color: 0xcda2a3, title: ctx.i18n`No builds for user` }] })
        );
      case 'get':
        if (build) {
          if (build.body) {
            return interaction.reply(withEphemeral(ctx.ephemerate, { embeds: [new BuildEmbed(build)] }));
          }
          const pages = this.#buildEmbedsForBuild(build, ctx);
          return Collectors.dynamic(interaction, pages, ctx);
        }
        const results = await ctx.settings.builds.getBuildSearch(query);
        if (results.length) {
          const pages = [];
        ...
        return interaction.reply(withEphemeral(ctx.ephemerate, { content: ctx.i18n`No builds found` }));
      case 'update':
      case 'add':
        ...
        if (build) {
          ...
          build = new Build(params as import('#shared/models/Build').BuildData, ctx.ws);
        }
        await ctx.settings.builds.saveBuild(build.toJson());
        const pages = this.#buildEmbedsForBuild(build, ctx);
        return Collectors.dynamic(interaction, pages, ctx);
      case 'remove':
        if (
          ...
          if (!thereWasAPart) await ctx.settings.builds.deleteBuild(id);
          else await ctx.settings.builds.saveBuild(build);
          return interaction.reply(withEphemeral(ctx.ephemerate, { content: 'buhleted' }));
        }
        break;
      case 'mod':
        if (!build) {
        ...
        const selectPartRow = () =>
          new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
              .setMinValues(0)
              .setMaxValues(1)
              .setCustomId('select_part')
              .setPlaceholder(ctx.i18n`Select Build Part`)
              .addOptions(
                populatedKeys
                  .map((k) => ({
                    value: k,
        ...
        let modPage = 0;
        const navComponents = new ActionRowBuilder().addComponents(
        ...
        const modChoices = () => {
          ...
          return [
            new ActionRowBuilder().addComponents(
              new StringSelectMenuBuilder()
                .setMinValues(0)
                .setMaxValues(available?.[modPage]?.length || 1)
                .setCustomId('select_mods')
                .setPlaceholder(ctx.i18n`Select Mods to Apply`)
                .setDisabled(!available?.length)
                .addOptions(
                  available?.[modPage]?.map((a) => ({
                    label: a.name,
                    value: a.uniqueName,
        ...
        const modSelectionHandler = async (selection) => {
          ...
          await interaction.editReply(
            withEphemeral(ctx.ephemerate, { content: currentMods(), components: [selectPartRow(), ...modChoices()] })
          );
        };
    ...
    return interaction.reply(withEphemeral(ctx.ephemerate, { content: ctx.i18n`Nah.` }));
  }
}
