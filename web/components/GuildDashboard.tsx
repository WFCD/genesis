...
const GuildDashboard: FC = () => {
  ...
  const loadCustomCommands = useCallback(
    async (reportError: boolean) => {
      ...
      if (res.ok) {
        setCustomCommands((await res.json()).commands ?? []);
        return;
      }
  ...
  const loadElevatedRoles = useCallback(
    async (reportError: boolean) => {
      ...
      if (res.ok) {
        setElevatedRoles(String((await res.json()).elevatedRoles ?? ''));
        return;
      }
      if (reportError) setLoadError(await readApiError(res));
    },
  ...
  const loadWelcomes = useCallback(
    async (reportError: boolean) => {
      ...
      if (res.ok) {
        setWelcomes((await res.json()).welcomes ?? []);
        return;
      }
      if (reportError) setLoadError(await readApiError(res));
    },
  ...
  const loadRooms = useCallback(
    async (reportError: boolean) => {
      ...
      if (res.ok) {
        setRooms((await res.json()).settings ?? {});
        return;
      }
      if (reportError) setLoadError(await readApiError(res));
    },
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (res.ok) setPingResults((await res.json()).results ?? []);
    }, 200);
